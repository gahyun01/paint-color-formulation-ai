...
이러한 데이터 불균형은 색상 영역에 따른 모델 성능 차이로 이어졌습니다. 데이터가 충분한 영역에서는 `ΔE76 0.5 이하`의 낮은 오차도 확인했지만, 학습 표본이 부족한 진한 파란색·보라색 영역에서는 모델이 색 혼합 관계를 충분히 학습하지 못해 오차가 크게 증가했습니다.
...
예시에서는 입력 LAB `[48.70, 20.37, 25.88]`에 대해 다음 배합을 예측했습니다.
...
재구성된 LAB는 `[48.75, 20.21, 25.90]`으로, 목표 색상과 유사한 결과를 확인했습니다.
...
- 일부 검증 사례에서 ΔE76 0.1705로 `ΔE76 0.5 이하`의 매우 낮은 오차 확인
